...
const UpdateInfo = () => {
    ...
    const handleSave = async (updatedData) => {
        ...
        if (user) {
            const docRef = doc(db, "users", user.uid);
            const newData = { ...userData, ...updatedData }; // Merge updates with existing data
            await updateDoc(docRef, newData); // Save to Firebase
            setUserData(newData); // Update local state
            setName(newData.name);
            setEmail(newData.email);
            setDisplayId(newData.displayId);
        }
                    ...
                    {showEditModel && (
                        <div className={s.modalOverlay}>
                            <div className={s.modalContent}>
                                <UpdatePage
                                    currentData={{ name, email, displayId }}
                                    onSave={handleSave}
                                />

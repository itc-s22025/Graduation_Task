...
const Keeps = () => {
    ...
    useEffect(() => {
        ...
        // FirestoreのユーザーごとのKeepsサブコレクションからデータを取得
        const keepsRef = collection(db, "users", user.uid, "keeps");
        const unsubscribe = onSnapshot(keepsRef, (snapshot) => {
            const keepsData = snapshot.docs.map((doc) => ({
                id: doc.id,
                ...doc.data(),
            }));
            setSavedPosts(keepsData);
            setKeepsIds(keepsData.map((post) => post.id));
        });
    ...
    const handleToggleSave = async (post) => {
        ...
        const postRef = doc(db, "users", user.uid, "keeps", post.id);
